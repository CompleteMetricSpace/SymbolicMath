...
public class Expr implements Serializable
{
		...
		if(this.isPower() && this.get(1).isRational())
		{
			...
			for(int i = 0;i<vars.length;i++)
			{
				...
				if(cd[1].isRational() && cd[0].equals(b))
				{
					...
					if(e.getNumerator().mod(d.getNumerator()).equals(Int.ZERO) && e.getDenominator().equals(d.getDenominator()))
	...
	public static int compare(Expr u, Expr v)
	{
        ...
        if(u.isFunctionForm() && v.isFunctionForm())
        {
        	if(!u.getOperator().equals(v.getOperator()))
        		return -u.getOperator().getName().compareTo(v.getOperator().getName());//u.getOperator().getPriority() >= v.getOperator().getPriority();
        	else
        	{
        		int m = u.length();
        		int n = v.length();
	...
	{
		return Simplification.simplify_recursive(new Expr(Operator.ADD, this, new Expr(Operator.MULTIPLY, Int.NONE, b)));
	}
	
	public Expr div(Expr b)
	{
		return Simplification.simplify_recursive(new Expr(Operator.MULTIPLY, this, new Expr(Operator.POWER, b, Int.NONE)));
	}
	...
	public Expr artanh()
	{
		return Rational.HALF.mul(new Expr(Operator.LOG, this.add(Int.ONE)).sub(new Expr(Operator.LOG, this.sub(Int.ONE))));
	}
