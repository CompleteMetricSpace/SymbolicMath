...
public class PSLQ 
{
    public static BigInteger[] pslq(Apfloat[] x, int prec, Apfloat epsilon)
    {
    	...
    	while(!done)
    	{
    		...
    		if(m <= n-2)
    		{
    			Apfloat t0 = ApfloatMath.sqrt(ApfloatMath.pow(H[m-1][m-1], 2).add(ApfloatMath.pow(H[m-1][m+1-1], 2)));
    ...
    {
    	System.out.println("pslq: "+Arrays.toString(pslq(new Apfloat[]{new Apfloat("1",70), new Apfloat("1.61833",70), new Apfloat("1.61833", 70).multiply(new Apfloat("1.61833",70))}, 70, new Apfloat("0.001",70))));
    }
}
