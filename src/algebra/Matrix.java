...
public class Matrix
{
    ...
    /*
     * Kochrezept 6.2-7 LA I/II Skript Dipper Returns matrix A and vector b in
     * 6.2-7-form and a set of column permutations used to obtain this form
     */
	...
	while(!done)
	{
	    ...
	    outloop: for(int i = c; i < A[0].length; i++)
	    {
		for(int j = r; j < A.length; j++)
		{
			...
			// Now, there is some nonzero element in A[r][c], divide
			// the whole row by it
			Expr d = A[r][c];
	...
	return Algebra.expand(Algebra.cancel(Matrix.determinant(sub(M,
		multiply_scalar(u, identity_matrix(M.length))))));
    }
     ...
     * 
     * @return list [k_1,...,k_r] where k_i is the number of jordan blocks of
     * size i
     */
	...
	for(int i = basis_vec.size() - 1; i >= 1; i--)
	{
	    ...
	    Expr[][] comp = LinearAlgebra.complete_basis(s, basis_vec.get(i - 1),
		    basis_vec.get(i - 1).length - s.length);
	    s = Set.add(s, comp);
	    jordan.add(0, s);
	}
