...
public class Algebra
{
	...
	if(u.isSum())
	{
	    ...
	    Expr[] r = u.rest();
	    return expand(v).add(
		    expand(Simplification.simplify_recursive(new Expr(Operator.ADD, r))));
	}
	if(u.isProduct())
	{
	    Expr v = u.get(0);
	    Expr[] r = u.rest();
	    return expand_product(expand(v),
		    expand(Simplification.simplify_recursive(new Expr(Operator.MULTIPLY, r))));
	}
	if(u.isPower())
	{
	    Expr b = u.get(0);
	...
	if(u.getOperator().equals(Operator.ADD) && n.isPositive())
	{
		...
		s[k.toInt()] = expand_product(expand(c.mul(expand_power(exp_1, n.sub(k)))),
			expand_power(exp_2, k));
	    }
	...
	if(u.getOperator().equals(Operator.ADD) && n.isPositive())
	{
		...
		return expand_product(
			u,
			expand_power_binary(expand_power(u, Int.TWO), n.sub(Int.ONE)
				.divide(Int.TWO)));
	    }
	}
	return u.pow(n);
	...
	if(u.isProduct())
	{
	    Expr v = u.get(0);
	    Expr[] r = u.rest();
	    return expand_product(weak_expand(v),
		    weak_expand(Simplification.simplify_recursive(new Expr(Operator.MULTIPLY, r))));
	}
	if(u.isPower())
	{
	    Expr b = u.get(0);
	...
	if(u.isProduct())
	{
	    Expr v = u.get(0);
	    Expr[] r = u.rest();
	    Expr[] nd_u_v = NumeratorDenominator(
		    Simplification.simplify_recursive(new Expr(Operator.MULTIPLY, r)), fraction);
	...
	if(nd_u[1].equals(Int.ONE) && nd_v[1].equals(Int.ONE))
	    ...
	    return rationalize_sum(nd_u[0].mul(nd_v[1]), nd_u[1].mul(nd_v[0])).div(
		    nd_u[1].mul(nd_v[1]));
    }
	...
	Expr n = nd[0];
	Expr d = nd[1];
	Expr[] vars = Set.union(Poly.variables_fractional_exponent(n),
		Poly.variables_fractional_exponent(d));
	if(vars.length == 0)
	    return n.div(d);
	Expr[] x = Simplification.getNewVariables(vars.length, n, d);
	n = Algebra.expand(Simplification.simplify_recursive(n.substitute(vars, x)));
	...
	for(int i = 0; i < lcm.length; i++)
	    lcm[i] = Int
		    .lcm(get_fraction_exponent_lcm(n, x[i]), get_fraction_exponent_lcm(d, x[i]));
