...
public class Simplification
{
	...
	public static Rule[] simplify_rules = 
		{
		    ...
		    new Rule(Parser.parse("multiply(_x, 1)"), Parser.parse("_x"), new Var[]{new Wild("_x")}),
		    new Rule(Parser.parse("multiply(1, _x)"), Parser.parse("_x"), new Var[]{new Wild("_x")}),
		    new Rule(Parser.parse("add(_x, _x)"), Parser.parse("multiply(2, _x)"), new Var[]{new Wild("_x")}),
		    new Rule(Parser.parse("add(_x, multiply(_n1, _x))"), Parser.parse("multiply(add(_n1, 1), _x)"), new Var[]{new Wild("_x"), new Var("_n1")}),
		    new Rule(Parser.parse("add(multiply(_n2, _x), multiply(_n1, _x))"), Parser.parse("multiply(add(_n2, _n1), _x)"), new Var[]{new Wild("_x"), new Var("_n1"), new Var("_n2")})
		};
	
	private static boolean expand_constants = true, simplify_rational_int_powers = true;
	private static Operator[] no_op_simplify = {Operator.HOLD, Operator.TIME, Operator.TABLE, Operator.LAZYSERIES};
	...
	public static Expr simplify_functions(Expr expr)
	{
		...
		{
			if(Programs.prgms[i].getName().equals(op.getName()) && Programs.prgms[i].getParameters().length == expr.length())
			{
				try
				{
	...
	private static Expr simplify_laplace_transform(Expr a, Expr b, Expr c) 
	{
		...
		{
			InOut.parser_output("[LaplaceTransform] Maximum recursion limit reached: "+e.getLimit());
			return Constant.FAIL;
		}
	}
	...
	private static Expr simplify_cofactor_matrix(Expr a) 
	{
		if(a.isMatrix())
		{
			return new Expr(Operator.MATRIX, Set.toList(Matrix.cofactor_matrix(Set.toExprArray(a.get(0)))));
		}
	...
	private static Expr simplify_transpose(Expr a) 
	{
		if(a.isMatrix())
		{
			return new Expr(Operator.MATRIX, Set.toList(Matrix.transpose(Set.toExprArray(a.get(0)))));
		}
		return new Expr(Operator.TRANSPOSE, a);
	}
	...
	private static Expr simplify_table(Expr[] p) 
	{
		...
		if(c.isInt() && d.isInt())
		{
			...
			if(F.compareTo(new Int(Integer.MAX_VALUE)) > 0)
				throw new IllegalArgumentException("difference between "+C+" and "+D+" is too big");
	...
	private static Expr simplify_integer_chinese_remainder(Expr e)
	{
		if(e.isList())
		{
			...
			{
				if(x[i].isList() && x[i].length() == 2 && x[i].get(0).isInt() && x[i].get(1).isInt())
				{
					Int a = (Int)x[i].get(0);
					Int b = (Int)x[i].get(1);
					M[i] = b;
					if(!b.isNaturalNumber())
						throw new IllegalArgumentException("modulus "+b+" is not a positive integer");
					X[i] = a.mod(b);
				}
				else
	...
	private static Expr simplify_simplify_side_relations(Expr a, Expr b, Expr c)
	{
		...
		if(!Poly.is_poly(a, vl))
			throw new IllegalArgumentException(a+" is not a multivariate polynomial in Q"+Expr.toString(vl));
		for(int i = 0;i<sr.length;i++)
			if(!Poly.is_poly(sr[i], vl))
				throw new IllegalArgumentException(sr[i]+" is not a multivariate polynomial in Q"+Expr.toString(vl));
	...
	private static Expr simplify_integrate_heuristic(Expr u, Expr x)
	{
		...
		{
			InOut.parser_output("[HeuristicIntegrate] Maximum recursion limit reached: "+e.getLimit());
			return Constant.FAIL;
		}
	}
	...
	private static Expr simplify_polynomial_extended_gcd(Expr list, Expr x)
	{
		if(list.isList())
		{
			...
			if(Poly.are_poly(l, new Expr[]{x}))
			{
				...
				throw new IllegalArgumentException("<pextgcd>: "+list+" are not polynomials in "+x);
		}
	...
	private static Expr simplify_resultant(Expr u, Expr v, Expr x) 
	{
		...
		if(Poly.is_poly(u, x) && Poly.is_poly(v, x))
		{
			...
			throw new IllegalArgumentException("<resultant>: "+u+" or "+v+" is/are not polynomials in "+x);
	}
			...
			if(list[0].isList() && list[1].getOperator().equals(Operator.MODULO))
			{
				...
				for(int i = 0;i<ps.length;i++)
				{
					...
					if(!Poly.is_poly_Z(ps[i], vars_i))
						throw new IllegalArgumentException(ps[i]+" is not a polynomial with coefficients in Z_"+m);
	...
	private static Expr simplify_risch_integration(Expr f, Expr x) 
	{
		...
		if(!RischIntegrate.is_exp_log_function(f, x))
			throw new IllegalArgumentException(f + " is not a pure transcendental exp-log-elementary function");
	...
	private static Expr simplify_gosper_integration(Expr f, Expr x) 
	{
		...
		if(ab == null)
			throw new IllegalArgumentException(f+" is apparently not a hypergeometric term over Q("+x+")");
	...
	private static Expr simplify_gosper_summation(Expr[] p) 
	{
		...
		if(uv == null)
			throw new IllegalArgumentException(a+" is apparently not a hypergeometric term over Q("+k+")");
	    ...
	    else
	    {
	    	...
	    	if(option.getOperator().equals(Operator.MODULO))
	    	{
	    		...
	    		if(P.isInt())
	    		{
	    			...
	    			Expr[] vars = Poly.variables(u);
	    			if(vars.length > 1)
	    				throw new IllegalArgumentException("Cannot factor a multivariate polynomial over a finite field");
	    			if(!Poly.is_poly_Z(u, vars))
	    				throw new IllegalArgumentException(u+" is not a polynomial with coefficients in Z_"+m);
	    		    if(vars.length == 0)
	    		    	return u;
	    	...
	    	if(option.getOperator().equals(Operator.ALGEBRAIC_EXTENSION))
	    	{
	    	    ...
	    	    if(m.isList())
	    	    	throw new IllegalArgumentException("Cannot factor over a non-simple algebraic extension"); 
	    	    Expr[] vars = Set.complement(Poly.variables(u), new Expr[]{alpha});
	    	    if(vars.length == 0)
	    	    	return u;
	    	    if(vars.length > 1)
	    	    	throw new IllegalArgumentException("Cannot factor a multivariate polynomial over an algebraic number field"); 
	    	    Expr x = vars[0];
	...
	private static Expr simplify_poly_quotient(Expr[] e) 
	{
		...
		else
		{
			...
			if(m.getOperator().equals(Operator.ALGEBRAIC_EXTENSION))
			{
			    ...
			    if(Poly.is_poly_Q(a, Set.add(new Expr[]{x}, alpha)) && Poly.is_poly_Q(b, Set.add(new Expr[]{x}, alpha)))
			        return AlgebraicNumberField.polynomial_quotient(a, b, x, p, alpha);
			    else
			    	throw new IllegalArgumentException("Cannot compute in a rational function field with algebraic extension");
			}
	...
	private static Expr[] simplify_intersection_rec(Expr[] L)
	{
				...
				&& !L[1].getOperator().equals(Operator.INTERSECTION))
		{
			if(L[0].isSet() && L[1].isSet())
			{
				if(L[0].length() == 1 && L[1].length() == 1)
				{
					...
					Expr intersect = new Expr(Operator.SET, new Expr(Operator.LIST, Set.intersect(list_1, list_2)));
		...
		if(L.length == 2)
		{
			if(L[0].getOperator().equals(Operator.INTERSECTION) && L[1].getOperator().equals(Operator.INTERSECTION))
			{
	...
	private static Expr[] simplify_union_rec(Expr[] L)
	{
				...
				&& !L[1].getOperator().equals(Operator.UNION))
		{
			if(L[0].isSet() && L[1].isSet())
			{
				if(L[0].length() == 1 && L[1].length() == 1)
				{
					Expr[] list_1 = L[0].get(0).getOperands();
					Expr[] list_2 = L[1].get(0).getOperands();
					Expr union = new Expr(Operator.SET, new Expr(Operator.LIST, Set.union(list_1, list_2)));
		...
		if(L.length == 2)
		{
			if(L[0].getOperator().equals(Operator.UNION) && L[1].getOperator().equals(Operator.UNION))
			{
	...
	private static Expr[] simplify_sum_rec(Expr[] L)
	{
		if(L.length == 2 && !L[0].isSum() && !L[1].isSum())
		{
			...
			if(L[0].getOperator().equals(Operator.PSERIES) && L[1].getOperator().equals(Operator.PSERIES))
			{
				if(L[0].get(1).equals(L[1].get(1)))
				{
			    	Series c = new Series(L[0].get(0).getOperands()).add(new Series(L[1].get(0).getOperands()));
			    	return new Expr[]{new Expr(Operator.PSERIES, new Expr(Operator.LIST, c.getCoefs()), L[0].get(1))};
				}
			}
			...
			if(L[1].isList() && L[0].isList() && L[1].length() == L[0].length())
			{
				...
				for(int i = 0;i<p.length;i++)
					p[i] = simplify_sum(new Expr(Operator.ADD, new Expr[]{L[0].get(i), L[1].get(i)}));
	...
	private static Expr[] simplify_product_rec(Expr[] L)
	{
				...
				&& !L[1].getOperator().equals(Operator.MULTIPLY))
		{
		    ...
		    if(L[0].getOperator().equals(Operator.BIG_O) && L[1].getOperator().equals(Operator.BIG_O))
		    {
		    	if(L[0].get(1).equals(L[1].get(1)))
		    	{
			...
			if(gr_0 != null && gr_1 != null)
			{
				...
				return new Expr[]{new Expr(Operator.ADD, new Expr[]{j[0], new Expr(Operator.MULTIPLY, new Expr[]{j[1],Constant.I})})};
			}
			...
			if(L[1].isList() && L[0].isList() && L[1].length() == L[0].length())
			{
				Expr[] p = new Expr[L[1].length()];
				for(int i = 0;i<p.length;i++)
					p[i] = simplify_product(new Expr(Operator.MULTIPLY, new Expr[]{L[0].get(i), L[1].get(i)}));
				return new Expr[]{new Expr(Operator.LIST, p)};
			}
			if(L[0].isList() && !L[1].isList())
			{
				...
				for(int i = 0;i<p.length;i++)
					p[i] = simplify_product(new Expr(Operator.MULTIPLY, new Expr[]{L[0].get(i), L[1]}));
				return new Expr[]{new Expr(Operator.LIST, p)};
			}
			if(L[1].isList() && !L[0].isList())
			{
				Expr[] p = new Expr[L[1].length()];
				for(int i = 0;i<p.length;i++)
					p[i] = simplify_product(new Expr(Operator.MULTIPLY, new Expr[]{L[0], L[1].get(i)}));
				return new Expr[]{new Expr(Operator.LIST, p)};
			}
			if(L[0].isMatrix() && L[1].isMatrix())
			{
				Expr[][] C = Matrix.multiply(Set.toExprArray(L[0].get(0)), Set.toExprArray(L[1].get(0)));
				return new Expr[]{new Expr(Operator.MATRIX, Set.toList(C))};
			}
			if(L[0].isMatrix() && !L[1].isMatrix())
			{
			...
			if(L[0].getOperator().equals(Operator.PSERIES) && L[1].getOperator().equals(Operator.PSERIES))
			{
				if(L[0].get(1).equals(L[1].get(1)))
				{
			    	Series c = new Series(L[0].get(0).getOperands()).mul(new Series(L[1].get(0).getOperands()));
			    	return new Expr[]{new Expr(Operator.PSERIES, new Expr(Operator.LIST, c.getCoefs()), L[0].get(1))};
				}
			}
			if(L[0].equals(Int.ONE))
		    ...
		    //Ints with same powers are collected together: 2^(1/3)*5^(1/3)*4 -> 10^(1/3)*4
		    if(be_1[0] instanceof Int && be_2[0] instanceof Int)
		    {
		    			...
		    			&& ((Int)be_2[0]).compareTo(Int.ZERO) == 1)
		    	{
		    		if(be_1[1].equals(be_2[1]))
		    		{
		    		    Expr P = simplify_product(new Expr(Operator.MULTIPLY, new Expr[]{be_1[0], be_2[0]}));
		    		    if(P.equals(Int.ONE))
		    		    	return new Expr[]{};
		    		    else
		    		    {
		    		    	//TODO: 2/15*2^(1/3)*3^(1/3)*5^(1/3)*7^(2/3) is not merged appropriately
		    		    	Expr r = simplify_power(new Expr(Operator.POWER, new Expr[]{P, be_1[1]}));
		    		...
		    		else
		    		{
		    			...
		    			if(!c.equals(Int.ONE))
		    			{
		    				Expr exp = simplify_sum(new Expr(Operator.ADD, new Expr[]{be_1[1], be_2[1]}));
		    			    Expr f = simplify_power(new Expr(Operator.POWER, new Expr[]{c, exp}));	
		    				a = a.divide(c);
		    				b = b.divide(c);
		    				Expr a_t = simplify_power(new Expr(Operator.POWER, new Expr[]{a, be_1[1]}));
		    				Expr b_t = simplify_power(new Expr(Operator.POWER, new Expr[]{b, be_2[1]}));
		    				Expr d = simplify_product(new Expr(Operator.MULTIPLY, new Expr[]{a_t, b_t, f}));
		    				if(d.equals(Int.ONE))
		    					return new Expr[]{};
		...
		if(L.length == 2)
		{
			if(L[0].getOperator().equals(Operator.MULTIPLY) && L[1].getOperator().equals(Operator.MULTIPLY))
			{
	...
	public static Expr simplify_power(Expr u)
	{
		...
		if(v.isProduct() && is_positive(v.get(0)))
		{
			...
			Expr d = simplify_power(new Expr(Operator.POWER, new Expr[]{simplify_product(new Expr(Operator.MULTIPLY, v.rest())), w}));
	...
	private static Expr simplify_num_fraction_power(Numerical v, Rational w) 
	{
		if(v.compareTo(Int.ZERO) == -1 && w.equals(new Rational(Int.ONE, Int.TWO)))
		{
			...
			return Simplification.simplify_product(new Expr(Operator.MULTIPLY, new Expr[]{P, Constant.I}));
		}
	...
	private static Expr simplify_fraction_fraction_power(Rational v, Rational w)
	{
		...
		return simplify_product(new Expr(Operator.MULTIPLY, new Expr[]{m, n_u, new Rational(Int.ONE, n), c}));
	}
	...
	private static Expr simplify_Int_fraction_power(Int v, Rational w)
	{
		...
		if(fac.length == 1)
		{
			...
			if(pow instanceof Int)
			{
				...
				return simplify_product(new Expr(Operator.MULTIPLY, new Expr[]{simplify_integer_pow(fac[0][0], (Int)pow), c}));
			}
			else
			{
			    if(fac[0][1].equals(Int.ONE))
			    {
				    ...
				    return simplify_product(new Expr(Operator.MULTIPLY, new Expr[]{new Expr(Operator.POWER, new Expr[]{fac[0][0], pow}), c}));
			    }
			    else
			    {
			    	if(c.equals(Int.ONE))
			            return simplify_Int_fraction_power(fac[0][0], (Rational)pow);
				    return simplify_product(new Expr(Operator.MULTIPLY, new Expr[]{simplify_Int_fraction_power(fac[0][0], (Rational)pow), c})); 	
			    }
		    }	
		}
		Expr[] prod;
		...
		{
			prod[i] = simplify_power(new Expr(Operator.POWER, new Expr[]{fac[i][0], fac[i][1].mul(rest)}));
		}
		if(!c.equals(Int.ONE))
	...
	private static Expr simplify_integer_pow(Expr v, Int n)
	{
		...
		if(v.isMatrix())
		{
			try 
			{
			    return new Expr(Operator.MATRIX, Set.toList(Matrix.power(Set.toExprArray(v.get(0)), n)));
			}
    ...
    private static Expr simplify_gr_pow(Numerical[] gr, Int n)
    {
		if(n.compareTo(Int.ZERO)>=0)
		{
			...
			Expr P = Simplification.simplify_product(new Expr(Operator.MULTIPLY, new Expr[]{j[1], Constant.I}));
		...
		{
			n = n.mul(Int.NONE);
			Numerical[] j = Numerical.div(new Numerical[]{Int.ONE, Int.ZERO},Numerical.pow(gr, n));
			Expr P = Simplification.simplify_product(new Expr(Operator.MULTIPLY, new Expr[]{j[1], Constant.I}));
			Expr S = Simplification.simplify_sum(new Expr(Operator.ADD, new Expr[]{j[0], P}));
			return S;
		}
	...
	public static Expr[] constant_mod_term(Expr e)
	{
		if(e.isProduct())
		{
			...
			for(int i = 0;i<e.length();i++)
			{
				if(e.get(i).isNumerical() || e.get(i) instanceof ModIntSym || e.get(i) instanceof ModIntNon)
					n = n.mul(e.get(i));
				else
					v.add(e.get(i));
			}
