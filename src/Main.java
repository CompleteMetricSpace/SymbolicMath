...
public class Main
{
	...
	{
		/*
		Expr n = Parser.build("x*(x+1)*((x^2*exp(2*x^2)-log(x+1)^2)^2+2*x*exp(3*x^2)*(x-(2*x^3+2*x^2+x+1)*log(x+1)))");
		Expr d = Parser.build("((x+1)*log(x+1)^2-(x^3+x^2)*exp(2*x^2))^2");	
		
		Expr n2 = Algebra.expand(Parser.build("sqrt(11)/2*x"));
		Expr d2 = Algebra.expand(Parser.build("t^2+1/11"));	
		
		Expr n3 = Simplification.simplify_recursive(n2.substitute(new Expr[]{x, new Var("t1"), new Var("t2")}, new Expr[]{th_0, th_1, th_2})); 
		Expr d3 = Simplification.simplify_recursive(d2.substitute(new Expr[]{x, new Var("t1"), new Var("t2")}, new Expr[]{th_0, th_1, th_2})); 
		

		Expr p1 = Parser.build("-2*th0*th1^2-4*th0^3*th1^2+2*th2/(1+th0)");
		Expr p2 = Parser.build("-th0^2*th1^2+th2^2");
		Expr p3 = Parser.build("-2*th0^3*th1^3/(1+th0)-(-2*th0^2*th1^3-4*th0^4*th1^3)*th2");
		
		Expr r = Parser.build("x^4+3*x^3+3*x^2-3*x+1");
		Expr f = Parser.build("exp(x^2+x-3)-2");
		LaurentSeries ls1 = new LaurentSeries(new Expr[]{Int.ONE, Int.TWO, Int.THREE}, new Int[]{new Int(-1), new Int(0), new Int(2)}, new Int(5));
		LaurentSeries ls2 = new LaurentSeries(new Expr[]{Int.ONE, Int.FOUR}, new Int[]{new Int(-5), new Int(-2)}, new Int(5));
         m{{1,2,3,4,5},{1,1,1,-3,0},{0,0,1,1,4},{1,1,4,1,1}}
*/
		Expr p = Parser.build("{{2,2,-1},{-1,-1,1},{-1,-2,2}}");
		Expr p2 = Parser.build("{3,3,1,3}");
		long time = System.currentTimeMillis();
		Expr[] A = new Expr[]{Int.ONE};
	   System.out.println("Dim ker: "+Set.toList(Matrix.jordan_basis(Set.toExprArray(p))));
		System.out.println("timed: "+(System.currentTimeMillis()-time));
       time = System.currentTimeMillis();
  // 	System.out.println("Denest: "+(Matrix.determinant(new Expr[][]{{new Var("a"), new Var("b"), new Var("c")}, {new Var("d"), new Var("e"), new Var("f")}, {new Var("g"), new Var("h"), new Var("i")}})));
       System.out.println("timed: "+(System.currentTimeMillis()-time));
	}
