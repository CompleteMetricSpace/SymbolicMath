...
public class GCD 
{
	...
	public static Expr modular_gcd_integers(Expr u, Expr v, Expr[] L)
	{
		...
		Int limit = (Int) Int.TWO.pow(n).mul(g.abs()).mul(Int.min(Poly.poly_height(A, L), Poly.poly_height(b, L)));
		...
		while(true)
		{
			...
			else if(m.equals(n) || (m.equals(Int.NONE) && n.isZero()) || (n.equals(Int.NONE) && m.isZero()))
			{
				Expr new_H = FiniteField.integer_chinese_remainder_garner(new Expr[]{H, Cp}, new Int[]{q, p});
				q = q.mul(p);
	...
	public static Expr modular_gcd_reduction(Expr A, Expr B, Expr[] L, Int p) 
	{
		...
		while(true)
		{
			...
			while(Simplification.simplify_recursive(g.substitute(x, bb)).equals(Int.ZERO) || Set.is_in_vector(q, bb));
		    ...
		    if(m.equals(n) || (m.equals(Int.NONE) && n.isZero()) || (n.equals(Int.NONE) && m.isZero()))
			{
				q.add(bb);
				H.add(Cb);
				Expr C = FiniteField.newton_interpolation(q.toArray(new Int[q.size()]), H.toArray(new Expr[H.size()]), x, p);
