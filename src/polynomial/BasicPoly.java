...
public class BasicPoly 
{
		...
		else
		{
			...
			while(m.compareTo(n)>=0)
			{
				...
				else
				{
					...
					if(!m.equals(Int.NONE))
					{
						q = FiniteField.to_non_negative(Algebra.expand(q.add(c.mul(x.pow(m.sub(n))))), p);
						r = FiniteField.to_non_negative(Algebra.expand(r.sub(c.mul(v).mul(x.pow(m.sub(n))))), p);
					}
					m = Poly.degree(r, x);
				}
			}
	...
	public static Expr[] pseudo_division(Expr u, Expr v, Expr x)
	{
		...
		return new Expr[]{Algebra.expand(lcv.pow(delta.sub(sigma)).mul(p)), Algebra.expand(lcv.pow(delta.sub(sigma)).mul(s))};
	}
	...
	public static Expr swinnerton_dyer_polynomial(Expr x, Int n)
	{
		...
		Expr s1 = Algebra.expand(Simplification.simplify_recursive(sdp.substitute(x, x.add(p.pow(Rational.HALF)))));
		Expr s2 = Algebra.expand(Simplification.simplify_recursive(sdp.substitute(x, x.sub(p.pow(Rational.HALF)))));
